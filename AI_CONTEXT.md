...
**Текущая версия**: 1.5 - Исправления публикации и улучшения UX
...
### **Немедленные действия для применения изменений (28.01.2026):**
...
2. **Обновить конфигурацию сессии** (если еще не сделано):
   - Открыть файл: `nano /ssd/www/youpub/config/env.php`
   - Найти строку: `'SESSION_LIFETIME' => 86400,`
   - Заменить на: `'SESSION_LIFETIME' => 36000, // 10 часов (36000 секунд)`
   - Сохранить и перезапустить PHP-FPM: `sudo systemctl reload php8.1-fpm`

3. **Проверить работу публикации**:
   - Проверить логи воркера: `tail -f /ssd/www/youpub/storage/logs/workers/smart_publish_worker.log`
   - Убедиться, что публикация проходит успешно
   - Проверить, что ссылки на YouTube используют формат шортсов
...
### **Приоритетные задачи для продолжения разработки:**
