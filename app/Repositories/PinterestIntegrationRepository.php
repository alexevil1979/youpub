...
class PinterestIntegrationRepository extends Repository
{
    ...
    public function findByUserId(int $userId): array
    {
        try {
            ...
            error_log("PinterestIntegrationRepository::findByUserId: Exception - " . $e->getMessage());
            try {
                $stmt = $this->db->prepare("SELECT * FROM `{$this->table}` WHERE user_id = ? ORDER BY created_at DESC");
                $stmt->execute([$userId]);
                return $stmt->fetchAll();
            } catch (\Exception $e2) {
                error_log("PinterestIntegrationRepository::findByUserId: Fallback also failed - " . $e2->getMessage());
                return [];
            }
        }
    }
    ...
    public function findDefaultByUserId(int $userId): ?array
    {
        try {
            ...
            error_log("PinterestIntegrationRepository::findDefaultByUserId: Exception - " . $e->getMessage());
            try {
                $stmt = $this->db->prepare("SELECT * FROM `{$this->table}` WHERE user_id = ? AND status = 'connected' ORDER BY created_at ASC LIMIT 1");
                $stmt->execute([$userId]);
                return $stmt->fetch() ?: null;
            } catch (\Exception $e2) {
                error_log("PinterestIntegrationRepository::findDefaultByUserId: Fallback also failed - " . $e2->getMessage());
                return null;
            }
        }
    }
